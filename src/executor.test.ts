...
return {
  ...
  const getLensIdToExplanationMap = (epi: any): Record<string, string> => {
    const composition = epi.entry?.find((e: any) => e.resource?.resourceType === 'Composition')?.resource;
    const extensions = composition?.extension || [];
    const appliedExtensions = extensions.filter(
      (ext: any) => ext.url === 'http://hl7.eu/fhir/ig/gravitate-health/StructureDefinition/LensesApplied'
    ...
    const entries = appliedExtensions.map((ext: any) => {
      const lensReference = ext.extension?.find((item: any) => item.url === 'lens')?.valueCodeableReference?.reference?.reference;
      const explanation = ext.extension?.find((item: any) => item.url === 'explanation')?.valueString;
      const lensId = typeof lensReference === 'string' ? lensReference.replace('Library/', '') : undefined;
      return [lensId, explanation] as const;
    }).filter((entry: readonly [string | undefined, string | undefined]) => typeof entry[0] === 'string') as Array<[string, string | undefined]>;

    return Object.fromEntries(entries.map(([lensId, explanation]) => [lensId, explanation || '']));
  };
  ...
  it('should store explanation generated by the same lens that is reported', async () => {
    const buildUniqueExplanationLens = (lensId: string, explanationText: string) => buildLens(
      lensId,
      `
return {
  enhance: () => html + '<!-- ${lensId} marker -->',
  explanation: () => '${explanationText}'
    ...
    const lenses = [
      buildUniqueExplanationLens('explain-lens-1', 'explain-lens-1 explanation'),
      buildUniqueExplanationLens('explain-lens-2', 'explain-lens-2 explanation')
    ];
    ...
    const explanationMap = getLensIdToExplanationMap(result.epi);
    expect(explanationMap['explain-lens-1']).toBe('explain-lens-1 explanation');
    expect(explanationMap['explain-lens-2']).toBe('explain-lens-2 explanation');
  });

  it('should allow empty explanation for applied lenses when lens returns empty text', async () => {
    const emptyExplanationLens = buildLens(
      'empty-explanation-lens',
      `
return {
  enhance: () => html + '<!-- empty explanation marker -->',
  explanation: () => ''
};
      `
    );

    const result = await applyLenses(JSON.parse(JSON.stringify(sampleEpi)), sampleIps, [emptyExplanationLens]);

    expect(result.focusingErrors.length).toBe(1);
    expect(result.focusingErrors[0].length).toBe(0);

    const explanationMap = getLensIdToExplanationMap(result.epi);
    expect(explanationMap['empty-explanation-lens']).toBe('');
  });
});
